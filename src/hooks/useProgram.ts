...
export function useProgram() {
  ...
  const getDayByIndex = (w: number, dayIndex: number) => {
    ...
    return {
      title,
      exercises,
      workSec: week.work_sec,
      restSec: week.rest_sec,
      rounds: week.rounds,
    };
  };

  /** Mark a day as done with an ISO timestamp */
  const markDone = async (w: number, title: string) =>
    localforage.setItem(EX_DONE_KEY(w, title), new Date().toISOString());

  /** Read the stored completion timestamp (or null if not done) */
  const getDoneStamp = async (w: number, title: string) =>
    (await localforage.getItem<string>(EX_DONE_KEY(w, title))) ?? null;

  /** Remove the stored completion stamp (unmark) */
  const unmarkDone = async (w: number, title: string) =>
    localforage.removeItem(EX_DONE_KEY(w, title));
  ...
  return {
    ...
    getDayByIndex,
    markDone,
    unmarkDone,   // <-- exported for Day.tsx
    getDoneStamp,
  };
}
